...
Focused Vitest covers strict recipe parsing and independent presets, all exposed controls, bounded plan/capabilities, phase continuity including negative wrap, RGB conversion and early adapter rejection before GPU access. `@wallet/ui` typecheck and scoped ESLint pass. The fallback is an explicit static graphite surface, not a CSS imitation of the running material.

An isolated, manual-frame WebGL2 probe lives in `probe/` and uses a temporary loopback server plus one headless Chromium context. It checks actual program link, complete RGBA8 FBO, distinct pixel profiles under identical colors/light, all 12 controls (drift separately), pause, 320/390/430/480 resize, 120×48 CSS-pixel button fill, five mount/dispose cycles and context-loss rejection. It writes `report.json` and four screenshots to the passed evidence directory, and closes browser and port 3146 in `finally`. Run only in ORACLE's GPU slot: `node packages/ui/src/background-sandbox/effects/vault-grid/probe/run.mjs [evidence-dir]` from the repository root.

The first successful probe used ANGLE/Vulkan SwiftShader. Its mean RGBA differences between identical-palette modes were tile/rib `72.00`, tile/engraved `13.60`, rib/engraved `73.71` (0–255 channels); zero drift/pause pixel delta was `0`, moving-light delta was `0.39`. There were no browser errors, and five resource cycles returned to the host-only baseline. These are software-renderer checks, not hardware/mobile FPS, final visual approval, context restoration or proof of the shared app compositor/real buttons. ORACLE owns those integration gates.

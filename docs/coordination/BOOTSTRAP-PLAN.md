...
### W4. Логотип Novex — ASSET READY / BLOCKED BY BASE

Владелец доработал логотип и передал `1111111.svg` как новую исходную версию. SHA-256: `439fde03f1a20cceac0f3402b7ff98182a732faf6907b09331ec740218f75da8`. SVG имеет прозрачный tight artboard `795×293`, не содержит scripts, handlers, внешних ресурсов, fonts или embedded data и безопасен как статический asset.

Новая версия заменяет прежний белый `1024×1024` source для runtime. Координаты отдельных путей выходят за нулевую границу меньше чем на `0.25px`; runtime-копии нужен прозрачный safety margin около `1px`, без изменения path geometry или палитры. Внедрять логотип следует отдельным MONO/V2 потоком в горизонтальный brand slot вместо временных `W` и `WALLET_4I7`, не в промежуточную PR #26 V1.

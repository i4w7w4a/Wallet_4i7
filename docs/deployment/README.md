# Изолированный деплой Novex Wallet на существующий VPS
...
> **СТОП: нижеследующий сценарий с `wallet.ru.net` — исторический, НЕ
> исполнять.** Регистратор сообщил, что домен занят другим лицом, покупку
> отменил. У нас нет подтверждённого права менять его DNS. Приведённые ниже
> site block, `A`-запись, rollback и preflight для этого имени — архив
> прежнего плана, не инструкция к действию. Новый постоянный домен потребует
> отдельной проверки владения и нового точного плана. Действующий preview —
> только `wallet.135.181.70.158.nip.io`.
...
Ниже сохранён **архив прежней процедуры** для Ubuntu 24.04, Docker 29.1.3 и Caddy 2.10. Её бывшая цель — показать `/` и `/mono` на `wallet.ru.net`, не меняя работающий сайт `dlc.ru.net` и админку (прямой порт `5174`). Домен `wallet.ru.net` не принадлежит владельцу проекта; нельзя выполнять шаги DNS/Caddy из этого архива. Приложение пока демонстрационное, не банковский production-сервис.
...
Отменённое предположение прежнего плана: `wallet.ru.net` считался новым apex с NS `ns1.nameself.com`/`ns2.nameself.com`, и планировалась `A`-запись на VPS. Регистратор опроверг владение; **не создавать эту запись и не применять** [`Caddyfile.wallet.snippet`](../../deploy/Caddyfile.wallet.snippet).

...
Перед реальным исполнением владелец подтверждает текущий полный release snapshot (включая незакоммиченный `/mono`), способ доставки, доступ к зоне, обслуживаемой указанными NS, и окно изменений. `git clone` одной ветки сейчас **не** воспроизводит вид приложения. Ранее раскрытые пароли не помещать в архив, Git, команды или отчёт; повторное использование допущено по решению владельца, ротация рекомендована отдельно и не является условием этой работы. Секреты приложению для текущей демоверсии не требуются.

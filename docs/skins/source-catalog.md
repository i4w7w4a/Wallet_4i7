...
## 12. CONTROL-FEEDBACK-01 — проверка одной компактной кнопки (2026-09-23)

Паспорта ниже начинались как исследование локального `/design-lab` на `codex/mono-release@6afcc5834624c1e9533a32ef29871b4603077813`. Проверены первичные исходники и тексты лицензий на указанных commit. Ни один upstream-компонент не скопирован и не установлен; Lab реализует собственный adapter на уже установленном `motion@13.2.0`. После просмотра владелец выбрал Material с `pressDepth: 2.7`, `magneticTravel: 5`, `settleMs: 270` для четырёх быстрых действий MONO. Этот вариант встроен как default; Magnetic остаётся кандидатом только для локальной примерки.

| Кандидат | Первичный source и license | Visual contract, адаптация и граница |
|---|---|---|
| **Material press · в Lab и четырёх действиях MONO** | [Cult UI `neumorph-button.tsx`](https://github.com/nolly-studio/cult-ui/blob/3b855612fb524cb042cc91b65f0cd575057471cc/apps/www/registry/default/ui/neumorph-button.tsx), commit `3b855612fb524cb042cc91b65f0cd575057471cc`, blob `3c697997b9df7cc95e31f6bf317562d137a66f76`; [MIT, Copyright 2023 Jordan-Gilliam](https://github.com/nolly-studio/cult-ui/blob/3b855612fb524cb042cc91b65f0cd575057471cc/LICENSE.md). Registry payload прочитан. Upstream требует `class-variance-authority`, `lucide-react`, `motion/react` и Tailwind. | В исходнике вместе меняются поверхность, inset shadow и press; есть `whileHover: 1.02`, `whileTap: 0.98`, spring `400/10`. В Lab сохранён связанный рельеф/press, убраны scale, demo-палитра, loader и новые зависимости; кнопка фиксирована `168×48`, внутренний ход ≤3 px, отклик без overshoot. В MONO собственный scoped CSS двигает внутренние icon/label слои на `2.7px`, возвращает их за `270ms` и сохраняет исходную геометрию плиток. Это самостоятельная реализация механики, не порт исходного TSX. Владелец просмотрел и утвердил её для этой четвёрки. |
| **Bounded magnetic · в Lab** | [Motion Primitives `magnetic.tsx`](https://github.com/ibelick/motion-primitives/blob/43a31886f505c0cd0f11887683cfb9ff36431f9c/components/core/magnetic.tsx), commit `43a31886f505c0cd0f11887683cfb9ff36431f9c`, blob `f9a04c694fa5a2ecfd65a018ac52c648b040f8dd`; [MIT, Copyright 2024 ibelick](https://github.com/ibelick/motion-primitives/blob/43a31886f505c0cd0f11887683cfb9ff36431f9c/LICENCE.md). Upstream использует `motion/react`, document `mousemove`, motion values/springs. | Сохранена связь отклонения с положением указателя и затухающий возврат. Изменено существенно: движется только текстовый слой, semantic button/hit target не смещается; слушатель привязан к самой кнопке, `mouse + fine hover` и reduced-motion fallback; предел хода 0–8 px. Нет document mousemove, постоянного RAF или нового renderer. Это не обещание полного поведения upstream `global`/`parent` action area. |
| **Perimeter shimmer · отложен** | [Magic UI `shimmer-button.tsx`](https://github.com/magicuidesign/magicui/blob/d7207e5692d14c00dceafa8488d6d01f197fa0e4/apps/www/registry/magicui/shimmer-button.tsx), commit `d7207e5692d14c00dceafa8488d6d01f197fa0e4`, blob `d675cc0979eb96b95b86ae0833c8065933aa21cd`; [MIT, Copyright Magic UI](https://github.com/magicuidesign/magicui/blob/d7207e5692d14c00dceafa8488d6d01f197fa0e4/LICENSE.md). [Registry JSON](https://github.com/magicuidesign/magicui/blob/d7207e5692d14c00dceafa8488d6d01f197fa0e4/apps/www/public/r/shimmer-button.json) включает `cssVars` и два бесконечных keyframes; TSX требует `cn` helper. | Исходная механика — движущийся свет по периметру. Для этой небольшой кнопки постоянный ambient отвлекает от press и не проходит дисциплину MONO; вариант не монтируется. При ином объекте потребуется отдельная проверка reduced motion и локализация CSS. |

Проверка React 19 / Next 16 / Tailwind 4 / Strict Mode относится к **нашему adapter** через тесты и локальный preview, а не к upstream-демо. Перенос Material ограничен четырьмя demo-действиями; магнитный вариант и другие объекты требуют отдельного визуального решения.

**Происхождение кода и notice.** Из Cult UI и Motion Primitives взяты только визуальные/поведенческие ориентиры. В текущих `control-feedback-button.tsx`, `control-feedback-button.module.css`, `control-feedback-model.ts`, `mono-quick-action-feedback.tsx` и `mono-interactions.css` нет скопированных upstream-файлов, фрагментов TSX/CSS, ассетов или новых зависимостей. Например, upstream Cult UI использует `whileTap` со scale и Tailwind variants, а наш Material — локальный press внутренних слоёв и собственные тени; upstream Magnetic использует document `mousemove` и motion values/springs, а наш расчёт ограничен bounds кнопки и отдельной нормализованной функцией. Тексты MIT этих источников требуют сохранять copyright/permission notice в копиях или существенных частях их Software. В этой интеграции такие части не переносились, поэтому отдельная запись для этих двух *референсов* в `THIRD_PARTY_NOTICES.md` сейчас не добавляется. Если позже будет импортирован их код или ассет, provenance и notice нужно пересмотреть до такого импорта.

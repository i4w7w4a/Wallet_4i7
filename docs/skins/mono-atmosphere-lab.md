...
- `MonoScene` integration uses its runtime-only `atmosphere` ReactNode slot and
  shared `surfaceRef`. The legacy DOM and pointer subscriptions are absent
  when the slot is supplied. `mono-atmosphere-lab-scene.tsx` uses the actual
  `MonoScene` with approved Ledger optics/Logo/Material and trusted demo snapshot.
  The context frame uses an outline so 390 means 390 CSS px, not 388; its nav
  is contained within the scrolling lab context. Chrome styles never target
...
The opaque base follows `--mono-palette-canvas-ff`; light shadows also use the
existing surface/border roles. `atmosphereCool` and `atmosphereWarm` tint the
material through a static color blend with their resolved alpha. The warm
channel uses the existing `-8a` paint token when a full-alpha paint token is not
provided. No central palette/token file is modified. This separates the
material's luminance/depth (recipe intensity) from palette hue/alpha, preserving
the authored contrast when the palette changes. Without palette tokens the
original graphite/ceramic recipe is used. Browser tests exercise two actual
normalized palettes at MonoScene's resolved CSS boundary and verify canvas,
cool and warm changes independently affect the background.
...
The targeted suite covers one backdrop, pointer/geometry separation, explicit
save/reload, A/B, undo, 320/390/430/480 widths, dark/light, coarse/reduced/saveData,
the real MonoScene (one Promo canvas, four unchanged Material targets, one Logo,
privacy retained through recipe switches), and resolved palette updates.

At 100% intensity, the neutral reading field and attenuated contours preserve
the measured balance/asset text contrast in static captures. With the actual
text color, transparent glyph captures, and the worst pixel under each text
box, minima across all four canonical widths were **4.67:1 light / 4.83:1 dark**.
This is a targeted check of 19 visible financial/asset text boxes per scene,
not a claim about every arbitrary user palette, position or optical pixel.
Decorative aria-hidden arrow glyphs are excluded. The audit PNG/JSON files are
generated by the final E2E case; original visual baselines are not overwritten.

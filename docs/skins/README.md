...
- Каждая секция rail сворачивается независимо. Общая команда левой rail сворачивает/разворачивает только `Варианты / Экран / Форма`; Color Lab и правая rail не затрагиваются. Отдельный master-переключатель скрывает обе rails без изменения draft.
...
- Preview-only Shape Lab редактирует радиус двух MONO group shells. У Ledger/Frost/Mercury независимые пробы; live preview не сохраняет пробу, `По умолчанию` возвращает встроенные значения, `Применить форму` переносит её в активный локальный рабочий пресет, `Отменить пробу формы` возвращает принятое значение. Старый ключ `shape-preview.v1` читается только при первичной миграции и не является product appearance.
- Первый слой Color Lab работает как дизайн-инструмент: круг `Основной цвет`, бегунки `Тон / Интенсивность`, четыре характера, `Цветовой перелив`, быстрые замки и `Новый вариант`. Dark и Light помнят свои настройки; один жест откатывается одним Undo. Сложные роли и seed остаются в `Точной настройке`, а старый архив палитр и сервер — за сервисным действием верхнего меню.
...
- Preview-only Material Lab встроен в правую rail: группы `Линза / Свет / Поле / Течение`, live sliders, `По умолчанию`, `Отменить пробу оптики` и `Применить`. Проба остаётся в памяти при collapse/hide; Apply переносит её в активный локальный рабочий пресет. Старый ключ `optical-preview.v1` читается при миграции. Approved built-in Default, V1 и product appearance не меняются.
...
- Тема `Тёмная / Светлая` и три фона `Ирис / Волна / Слои` — независимые оси внутри выбранного рабочего пресета. Тёмная среда получила слабый холодно-тёплый перелив; светлая — собственную pearl/ceramic грамматику, а не инверсию.
...
Новый верхний selector выбирает один активный локальный рабочий пресет оформления. `wallet4i7.mono.working-presets.v1` хранит его ID, название, ревизию, три цветовых slot с историей, принятую форму и оптику, тему и фон. Прямые изменения сохраняются автоматически после короткой задержки; A/B Compare не пишет storage. При неудаче записи остаются live draft, статус ошибки и экспорт. Переключение при непринятой пробе формы/оптики останавливается до Apply/Cancel. JSON выбранного пресета доступен по требованию: полный V2 импорт проходит проверку и preview, затем создаёт новую копию. Старый экспорт палитры принимается только как palette-only, остальные оси получают built-in defaults. Ключи старых библиотек не удаляются. `Применить палитру` по-прежнему отдельно пишет `palette-active.v2`, не публикует пресет и не меняет V1. Серверный архив доступен по явному действию и при HTTP 503 не мешает локальной работе. Font Lab, общий `SkinHost` и перенос product state пока не реализованы.

`localStorage` ограничен origin: preview на 3105 не увидит данные 3104 автоматически. Миграция старых ключей работает внутри одного origin; между портами пользователь переносит экспортированный JSON. UI 3104 экспортирует только текущую палитру, не форму/оптику/фон.

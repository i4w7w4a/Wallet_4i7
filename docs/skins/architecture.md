...
Состояние chrome (`panelsVisible`, открытая drawer, expanded sections) отделено от design state. Независимый collapse секции, общий collapse только трёх левых секций и master hide не вызывают Default или Apply. Локальный рабочий пресет `/mono` хранится в `wallet4i7.mono.working-presets.v1`: прямые цветовые и фоновые правки autosave-ятся, а optical/shape проба остаётся в памяти до явного Apply или Cancel. Apply формы и оптики обновляет только принятые значения активного рабочего пресета. Старые `shape-preview.v1` и `optical-preview.v1` остаются входами первичной миграции, не product active appearance.

...
Состояние chrome (`panelsVisible`, открытая drawer, expanded sections) отделено от design state. Независимый collapse секции и общий master hide только меняют оболочку: они не вызывают Default, Apply или восстановление storage. Optical draft остаётся в памяти для каждого Ledger/Frost/Mercury direction, live обновляет uniforms и записывается в preview storage только явным `Apply`.
